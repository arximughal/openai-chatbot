...
const generatePromptForPersonas = (message, persona) => {
    switch(persona?.trim().toLowerCase()) {
        ...
        case 'pamayres':
            return generatePromptForPamAyres(message);
        default:
            break;
...
const generatePromptForPamAyres = (prompt) => {
  return `Pretend that you are Pam Ayres. Answer with humourous ryhyming poetry.
    Person: ${prompt}
    Pam: `;
};
